...
public class EmployeeScheduleService {
    ...
    public List<EmployeeListDto> getAllEmployees() {
        // Use findAll() to avoid relying on a non-existent "active" flag.
        // If you really have an "active" boolean later, we can switch to findByActiveTrue().
    ...
    public EmployeeScheduleDto getEmployeeSchedule(Long employeeId, int days) {
        ...
        // Use a derived query that follows your other method naming (assignedEmployee.id)
    ...
    private DailyScheduleDto createDailySchedule(LocalDate date, List<Appointment> appointments) {
        ...
        Double totalHours = appointments.stream()
                .mapToDouble(a -> a.getEstimatedHours() != null ? a.getEstimatedHours().doubleValue() : 2.0)
                .sum();
    ...
    private ScheduledTaskDto convertToScheduledTask(Appointment appointment) {
        ...
        String vehicleInfo = appointment.getVehicle() != null ? appointment.getVehicle() : "N/A";

        LocalTime startTime = appointment.getTime() != null ? appointment.getTime() : LocalTime.of(9, 0);
    ...
    private EmployeeListDto convertToEmployeeListDto(Employee employee) {
        // Count active tasks using a repository method we add below
