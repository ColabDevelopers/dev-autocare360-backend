...
public interface TimeLogRepository extends JpaRepository<TimeLog, Long> {
  ...
  @Query(
      "SELECT COALESCE(SUM(t.hours), 0) FROM TimeLog t WHERE t.employee.id = :employeeId AND t.date = :date")
  BigDecimal sumHoursByEmployeeAndDate(
      @Param("employeeId") Long employeeId, @Param("date") LocalDate date);

  // Sum hours for employee in date range
  @Query(
      "SELECT COALESCE(SUM(t.hours), 0) FROM TimeLog t WHERE t.employee.id = :employeeId AND t.date BETWEEN :startDate AND :endDate")
  ...
  @Query(
      "SELECT COALESCE(SUM(t.hours), 0) FROM TimeLog t "
          + "WHERE t.employee.id = :employeeId AND t.appointment.id = :appointmentId")
  BigDecimal sumHoursByEmployeeAndAppointment(
      @Param("employeeId") Long employeeId, @Param("appointmentId") Long appointmentId);
